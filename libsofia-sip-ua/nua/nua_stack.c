	       ...
	       tag_type_t tag, tag_value_t value, ...)
{
  ...
  if (nua->nua_shutdown_started && event != nua_r_shutdown && event != nua_r_unref && event != nua_r_handle_unref)
    return -1;
...
void nua_stack_signal(nua_t *nua, su_msg_r msg, nua_ee_data_t *ee)
{
  ...
  if (nua->nua_shutdown && !e->e_always && event != nua_r_unref && event != nua_r_handle_unref) {
    /* Shutting down */
